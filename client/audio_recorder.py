...
def main():
    """Función principal - patrón Go."""
    
    # Validar argumentos
    """
    if len(sys.argv) != 2:
        print(f"Uso: {sys.argv[0]} <URL>")
        print(f"Ejemplo: {sys.argv[0]} 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'")
        sys.exit(1)
    """
    #url = sys.argv[1]
    url = "https://www.youtube.com/@todonoticias/live"

    log("🚀 Simple Audio Recorder - Patrón Go", "INFO")
    ...
    try:
        # 1. Crear sink de PulseAudio
        sink_name = create_audio_sink()
        if not sink_name:
            log("Error creando sink de audio", "ERROR")
            return 1
        
        # 2. Iniciar Firefox con sink configurado
        firefox_proc = start_firefox(url, sink_name)
        if not firefox_proc:
            log("Error iniciando Firefox", "ERROR")
            return 1
        
        # 3. Configurar dispositivo de captura
        pulse_device = f"{sink_name}.monitor"
        
        # 4. Iniciar grabación en hilo separado
        log("Iniciando sistema de grabación...")
        ...
        log("✅ Sistema iniciado. Grabando chunks de 15s...", "SUCCESS")
        log("Presiona Ctrl+C para detener")
        
        # 5. Mantener vivo hasta señal de parada
        try:
            while True:
                time.sleep(1)
