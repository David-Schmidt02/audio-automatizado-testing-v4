            ...
            try:
                while not audio_client_session.stop_event.is_set():
                    ...
                    if not data:
                        break
                    try:
                        send_pcm_to_server(data, audio_client_session.id_instance)
...
def main():
    ...
    if not sink_name:
        log("❌ Failed to create PulseAudio sink", "ERROR")
        audio_client_session.cleanup()
        sys.exit(1)
    ...
    if not firefox_profile_dir:
        log("❌ Failed to create Firefox profile", "ERROR")
        audio_client_session.cleanup()
        sys.exit(1)
    ...
    if audio_client_session.start_thread_ads_control():
